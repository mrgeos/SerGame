...
SG.Art = (function () {
  ...
  function gatePorch() {
    var o = cv(102, 70), g = o.g;
    var panel = '#4a4468', panelD = '#3b3656', panelL = '#5b5480';

    px(g, 0, 0, 102, 70, panel);                  // стена панельки
    px(g, 0, 0, 102, 2, panelL);
    for (var s = 12; s < 70; s += 18) px(g, 0, s, 102, 1, panelD);  // швы плит

    px(g, 17, 3, 14, 9, '#2f2a4a');               // окна лестничной клетки
    px(g, 18, 4, 12, 7, '#3a3560');
    px(g, 71, 3, 14, 9, '#2f2a4a');
    px(g, 72, 4, 12, 7, '#ffd583');

    px(g, 39, 8, 24, 7, P.out);                  // табличка с номерами квартир
    px(g, 40, 9, 22, 5, '#20304a');
    px(g, 42, 11, 3, 1, P.gold); px(g, 47, 11, 2, 1, P.gold);
    px(g, 51, 11, 1, 1, P.gold); px(g, 54, 11, 5, 1, P.gold);

    px(g, 15, 18, 72, 6, P.greyD);                // козырёк
    px(g, 15, 18, 72, 2, P.greyL);
    px(g, 17, 24, 68, 2, '#2f2b40');
    px(g, 47, 26, 8, 3, '#ffe9a8');              // плафон
    px(g, 48, 29, 6, 1, '#ffd583');

    px(g, 30, 24, 42, 46, P.woodD);              // дверной портал
    px(g, 33, 26, 36, 44, '#14111f');            // проём
    px(g, 33, 26, 36, 1, '#241d33');

    px(g, 73, 40, 9, 14, P.greyD);               // домофон
    px(g, 74, 41, 7, 6, '#2b4056');
    px(g, 75, 42, 3, 2, '#7fb0d0');
    for (var r = 0; r < 3; r++)
      for (var c = 0; c < 3; c++)
        px(g, 74 + c * 3, 48 + r * 2, 2, 1, P.greyL);
  ...
  var HI = {
    ...
    gate_metro: 2.3,
    /* Подъезд — не просто дверь, а кусок дома Серёги в несколько этажей,
     * поэтому по ширине он скромнее прочих выходов: иначе не влезает
     * по высоте. Портал и створка тянутся одинаково — на этом держится
     * совпадение проёма со створками. */
    gate_porch: 1.8, gate_porch_leaf: 1.8
  };
