...
SG.Art = (function () {
  ...
  function gatePorch() {
    var o = cv(76, 70), g = o.g;
    var panel = '#4a4468', panelD = '#3b3656', panelL = '#5b5480';

    px(g, 0, 0, 76, 70, panel);                  // стена панельки
    px(g, 0, 0, 76, 2, panelL);
    for (var s = 12; s < 70; s += 18) px(g, 0, s, 76, 1, panelD);   // швы плит

    px(g, 4, 3, 14, 9, '#2f2a4a');               // окна лестничной клетки
    px(g, 5, 4, 12, 7, '#3a3560');
    px(g, 58, 3, 14, 9, '#2f2a4a');
    px(g, 59, 4, 12, 7, '#ffd583');

    px(g, 26, 8, 24, 7, P.out);                  // табличка с номерами квартир
    px(g, 27, 9, 22, 5, '#20304a');
    px(g, 29, 11, 3, 1, P.gold); px(g, 34, 11, 2, 1, P.gold);
    px(g, 38, 11, 1, 1, P.gold); px(g, 41, 11, 5, 1, P.gold);

    px(g, 2, 18, 72, 6, P.greyD);                // козырёк
    px(g, 2, 18, 72, 2, P.greyL);
    px(g, 4, 24, 68, 2, '#2f2b40');
    px(g, 34, 26, 8, 3, '#ffe9a8');              // плафон
    px(g, 35, 29, 6, 1, '#ffd583');

    px(g, 17, 24, 42, 46, P.woodD);              // дверной портал
    px(g, 20, 26, 36, 44, '#14111f');            // проём
    px(g, 20, 26, 36, 1, '#241d33');

    px(g, 60, 40, 9, 14, P.greyD);               // домофон
    px(g, 61, 41, 7, 6, '#2b4056');
    px(g, 62, 42, 3, 2, '#7fb0d0');
    for (var r = 0; r < 3; r++)
      for (var c = 0; c < 3; c++)
        px(g, 61 + c * 3, 48 + r * 2, 2, 1, P.greyL);
