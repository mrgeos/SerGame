...
SG.CFG = {
  ...
  /* Первая строка идёт и в баннер на старте забега */
  intro: [
    'Воскресенье. 18:59.',
  ...
  finale: {
    ...
    lines: ['Будь всегда счастлив и силён!']
  },
  ...
  hat: {
    ...
    /* Шапка работает один уровень: на выходе слетает и возвращает жизни.
     * Если и на новом уровне всё потеряно — Геос присылает новую, так что
     * проиграть подарок по-прежнему нельзя. */
    livesBack: 3
  },
  ...
  run: {
    ...
    shredMs: 6000,        // сколько длится режим «море по колено»
   ...
   * Девять кадров, листаются кнопкой. Картинки лежат в assets/comic/
   * как frame1.png … frame9.png и перечислены в assets/comic/manifest.json.
   *
   * Реплики кладутся поверх кадра теми же плашками, что и в игре.
   * Координаты — доли от размера кадра, считая от его левого верхнего угла,
   * поэтому не зависят от разрешения картинки. tail — куда смотрит хвостик
   * (доли ширины плашки: -1 влево, 0 вниз, 1 вправо).
   *
   * На шестом и восьмом кадрах реплики уже врисованы в картинку — там
   * ничего не накладываем. caption — подпись-врезка сверху, без хвостика. */
  comic: {
    enabled: true,
    next: 'ДАЛЬШЕ',
    skip: 'ПРОПУСТИТЬ',
    start: 'ПОЕХАЛИ!',
    frames: [
      { caption: 'Офис, где-то середина дня… 9 августа.' },
      {},
      { lines: [
        { text: 'Серёжа, поспеши домой, у нас сюрприз!', x: 0.79, y: 0.13 },
        { text: 'Бегу!', x: 0.10, y: 0.16, tail: 0.7 }
      ] },
      {},
      { lines: [
        { text: 'Сергей! Нужно СРОЧНО внести несколько правок.', x: 0.75, y: 0.12 },
        { text: 'Завтра сделаю, мне надо домой — сегодня воскресенье '
              + 'и у меня день рождения!', x: 0.24, y: 0.87, up: true },
        { text: 'Сергей, очень надо прямо сейчас!!!', x: 0.74, y: 0.90, up: true }
      ] },
      {},                       // «Я СКАЗАЛ — ЗАВТРА-А-А!!!» уже на картинке
      {},
      {},                       // «Посмотрим, как ты доберёшься!» уже на картинке
      {}
    ]
  },
